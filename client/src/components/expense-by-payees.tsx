...
import { getUniquePayeeNames, getTotalExpenseAmountByPayee, getGrandTotal } from "../services/expense-utils";
        ...
        {uniquePayeeNames.map((payeeName, index) => {
          return (
            <tr>
              <td>{index + 1}</td>
              <td>{payeeName}</td>
              <td>{getTotalExpenseAmountByPayee(payeeName, expenseItems)}</td>
            </tr>
          );
        })}
         <tr>
